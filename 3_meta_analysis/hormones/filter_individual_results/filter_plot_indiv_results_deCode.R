...
colnames(GWAS_res)[which(colnames(GWAS_res) == "Inf.")] <- "INFO"

if (grepl("^chr", GWAS_res$Chr[1])) {
  GWAS_res$Chr <- gsub("^chr", "", GWAS_res$Chr)
}
# Replace X with 23
GWAS_res$Chr[which(GWAS_res$Chr == "X")] <- 23
...
acgt_filter <- function (qc_log, dat) {
  res <- dat %>% filter(EA %in% c("A", "C", "G", "T") &
                          OA %in% c("A", "C", "G", "T"))
  sink(qc_log, append = T)
  cat(paste0("\t", 
...
duplicate_snps <- function (qc_log, dat) {
  res <- dat %>% 
    mutate(dup_check = paste0(Chr, ":", PosB38)) %>%
...
cleaned <- cleaned %>% arrange(Chr, PosB38) %>%
  mutate(ID = paste0("chr", Chr, ":", PosB38),
         N = "NA", CHISQ = "NA")

to_print <- cleaned[, c("ID", "Chr", "PosB38", 
                        "EA", "OA", "EAF", "MAF", 
                        "beta", "SEbeta", "CHISQ", "N", "P")]
colnames(to_print) <- c("ID", "CHROM", "GENPOS", 
                        "ALLELE1", "ALLELE0", "A1FREQ", "MAF",
                        "BETA", "SE", "CHISQ", "N", "PVALUE")
