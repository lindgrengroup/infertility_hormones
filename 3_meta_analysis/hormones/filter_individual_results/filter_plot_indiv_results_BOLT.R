...
to_numeric <- c("BP", "A1FREQ", "F_MISS", "CHISQ_LINREG", "P_LINREG",
                "BETA", "SE", "CHISQ_BOLT_LMM_INF", "P_BOLT_LMM_INF")
...
duplicate_snps <- function (qc_log, dat) {
  res <- dat %>% 
    mutate(dup_check = paste0(CHR, ":", BP)) %>%
...
cleaned <- cleaned %>% arrange(CHR, BP) %>%
  mutate(ID = paste0("chr", CHR, ":", BP),
         N = "NA")

to_print <- cleaned[, c("ID", "CHR", "BP", 
                        "ALLELE1", "ALLELE0", "A1FREQ", "MAF", 
                        "BETA", "SE", "CHISQ_BOLT_LMM_INF", "N", "P_BOLT_LMM_INF")]
