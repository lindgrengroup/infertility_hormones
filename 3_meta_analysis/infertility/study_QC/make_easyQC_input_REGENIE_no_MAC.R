...
OUTPUTDIR<-paste("/well/lindgren/laura/projects/infertility/easyQC/input_files",STUDY,sep="/")
...
print(paste("Number of SNPs by missing LOG10P filter",nrow(data)-nrow(data[!is.na(data$LOG10P),]),sep=":"))
data<-data[!is.na(data$LOG10P),]

# Drop duplicate chrpos
data$chrpos<-paste(data$CHROM,data$GENPOS,sep=":")
...
data2<-data[which(data$ALLELE0 %in% c("A","C","G","T")),]
data2<-data2[which(data2$ALLELE1 %in% c("A","C","G","T")),]
...
# data$maf<-ifelse(data$A1FREQ<=0.5,data$A1FREQ,1-data$A1FREQ)
...
data$alleles<-apply(cbind(data$ALLELE0, data$ALLELE1), 1, function(x) paste(sort(x), collapse="_"))
data$markername<-paste(data$chrpos,data$alleles,sep=":")

# Output QCed data
setwd(OUTPUTDIR)
OUTPUT_FILENAME<-paste(STUDY,paste(TRAIT,"no_MACfilter_QCed.txt",sep="_"),sep="_")
data<-data[,-which(names(data) %in% c("alleles","chrpos","EXTRA","CHISQ","TEST"))]
