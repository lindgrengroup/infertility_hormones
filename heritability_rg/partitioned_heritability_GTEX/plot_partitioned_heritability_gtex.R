...
celltypes_tissue <- read.table(paste0(main_filepath, "/cell_types_tissue_categories.txt"),
                               sep = "\t", header = T, stringsAsFactors = F)
celltypes_tissue <- celltypes_tissue %>%
  mutate(cts_name = gsub(" ", "_", cts_name),
         tissue_category = ifelse(tissue_category == "Musculoskeletal/connective",
                                  "Musculoskeletal/Connective", tissue_category),
         tissue_category = factor(tissue_category,
                                  levels = c("Adipose", "Blood/Immune",
                                             "CNS", "Cardiovascular",
                                             "Digestive", "Endocrine",
                                             "Liver", "Musculoskeletal/Connective",
                                             "Other")))

# European results
cts_EUR <- lapply(c(HORMONES, INFERTILITY), function (trait) {
  checkname <- paste0(main_filepath, "/gtex_heritability/",
                      trait, "_EUR_gtex.cell_type_results.txt")
...
cts_all_anc <- lapply(c(HORMONES, INFERTILITY), function (trait) {
  checkname <- paste0(main_filepath, "/gtex_heritability/",
                      trait, "_all_anc_gtex.cell_type_results.txt")
  if (file.exists(checkname)) {
    dat <- read.table(checkname,
                      sep = "\t", header = T)
    dat$trait <- trait
...
getEnrichmentPlot <- function (df) {
  res_plot <- ggplot(df, aes(x = Name, y = neglogp)) +
    ...
    geom_point(aes(size = neglogp, alpha = signif,
                   fill = tissue_category, colour = tissue_category)) +
    geom_text_repel(data = df %>% filter(signif == "signif"), 
                    aes(label = Name), 
                    show.legend = F) + 
...
ggsave(paste0(main_filepath, "/gtex_heritability/plots/hormones_EUR_cts.png"), 
       getEnrichmentPlot(cts_EUR_for_plot %>% filter(trait %in% HORMONES)),
       height = 14, width = 14, units = "in")

ggsave(paste0(main_filepath, "/gtex_heritability/plots/infertility_EUR_cts.png"), 
...
cts_EUR_testo <- cts_EUR %>% filter(trait == "Testosterone_F") %>%
  left_join(celltypes_tissue, by = c("Name" = "cts_name")) %>%
  mutate(neglogp = -log10(Coefficient_P_value),
         signif = ifelse(neglogp >= PTHRESH, "signif", "no")) %>%
  arrange(tissue_category, desc(neglogp)) 
...
ggsave(paste0(main_filepath, "/gtex_heritability/plots/testo_F_EUR_cts_barplot.png"), 
       getEnrichmentBarPlot(cts_EUR_testo),
       height = 5, width = 15, units = "cm")

cts_EUR_infert <- cts_EUR %>% filter(trait == "female_infertility_analysis1") %>%
  left_join(celltypes_tissue, by = c("Name" = "cts_name")) %>%
  mutate(neglogp = -log10(Coefficient_P_value),
         signif = ifelse(neglogp >= PTHRESH, "signif", "no")) %>%
  arrange(tissue_category, desc(neglogp)) 
...
# Order of names for plot
name_order <- unique(cts_EUR_infert$Name)
cts_EUR_infert$Name <- factor(cts_EUR_infert$Name,
                             levels = name_order)

ggsave(paste0(main_filepath, "/gtex_heritability/plots/female_infertility_analysis1_EUR_cts_barplot.png"), 
       getEnrichmentBarPlot(cts_EUR_infert),
       height = 5, width = 15, units = "cm")
